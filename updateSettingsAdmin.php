   ...
   $conn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD,DB_NAME)
    ...
    if($action == 1){
    	...
    	if($_REQUEST["recid"]!= ""){   		   		
    		...
    		$Query = "UPDATE Recruitments1 SET isfinanceverified='1'  WHERE id='".$currRecruitmentId."'";
			//echo $Query;
			$insertQueryRes = mysqli_query($conn,$Query); //Inserts the value to table Student				
			if (!$insertQueryRes) {				
				echo "fail";				
	    ...
	    if($_REQUEST["recid"]!= ""){   		   		
	    	$currRecruitmentId = intval(@trim(stripslashes($_REQUEST["recid"])));
	    	$Query = "Select isfinanceverified from Recruitments1 WHERE id='".$currRecruitmentId."'";
	    	//echo $Query;
    	...
    	if($_REQUEST["sem"]!= "" && $_REQUEST["accyear"]!=""){
    		...
    		$oduRRF = @trim(stripslashes($_REQUEST["oduRRF"]));
	    	$QueryGet = "Select startdate,enddate from AdminSemDateSettings WHERE semester='".$curSem."' and year='".$accYear."' and odurrf=".$oduRRF;
	    	//echo $Query;
			...
			$num_rows = mysql_num_rows($result);
				...
				$QueryInsert = "INSERT INTO AdminSemDateSettings(semester, startdate, enddate, year, updatedby, odurrf) VALUES ('$curSem','$defStartDate','$defEndDate','$accYear',$user_id,$oduRRF)";
				//echo $QueryInsert;
    	...
    	if($_REQUEST["curStartDate"]!= "" && $_REQUEST["curEndDate"]!=""){   
    		...
    		$QueryUpdate = "UPDATE AdminSemDateSettings SET startdate='".$defStartDate."' , enddate='".$defEndDate ."' where semester='".$curSem."' and odurrf=".$oduRRF." and year='".$accYear."'";
			//echo $QueryUpdate;
    		$QueryRes = mysqli_query($conn,$QueryUpdate); //Inserts the value to table Student		
			//echo "QueryRes=".$QueryRes;
