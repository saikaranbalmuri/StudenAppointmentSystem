...
*/
	...
	$stmt = $conn->prepare('SELECT R.student_id AS stu_id, Stu.firstname as stu_fn, Stu.lastname AS stu_ln,Stu.email AS stu_email,
	R.currentpost,R.tutionwaive,R.credithours,R.semester,R.year,Sta.faculty_id as sta_id,
	 Sta.firstname As sta_fn,Sta.lastname AS sta_ln,Sta.email AS sta_email,
	  R.isreappointed FROM Recruitments1 R JOIN Student Stu ON R.student_id = Stu.uin JOIN Staff Sta ON R.faculty_id = Sta.faculty_id where R.id= ?');
    ...
    while($row = $result->fetch_assoc()) {  
		...
		$Query = "UPDATE Recruitments1 SET offerstatus='3' WHERE id=".$recruitmentId;		
