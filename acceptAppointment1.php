<?php
	...
	include ('connect.php');
	session_start();
	$recruitmentIdStr = $_SESSION["recruitmentId"];
...
/*	$stmt = $conn->prepare('SELECT R.student_id AS stu_id, Stu.firstname as stu_fn, Stu.lastname AS stu_ln,Stu.email AS stu_email,R.id as rec_id,
	R.currentpost,R.semester,R.year,R.salarypaid,R.fundingtype,Sta.faculty_id as sta_id,
	...
	$stmt->bind_param('s', $recruitmentIds[0]);
	$stmt->execute();
	$result = $stmt->get_result();

	    while($row = $result->fetch_assoc()) {  
	    	    	
			$Query = "UPDATE Recruitments1 SET offerstatus='2' WHERE id=".$row["rec_id"];				
			//echo $Query;
