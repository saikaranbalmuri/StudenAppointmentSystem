			    ...
			    if($exists > 0) //IF there are no returning rows or no existing username
			    {
			       while($row = mysql_fetch_assoc($query)) // display all rows from query
			       {
			          ...
			          $table_password = $row['password']; // the first password row is passed on to $table_password, and so on until the query is finished
			       	  ...
			       	  $loginUser_FullName = $row['lastname'].",".$row['firstname'];
			 		  //echo '<script>alert("result:'.$loginUser_FullName .'");</script>';	            		       	  
			         $isAdmin = False;
			       	  $adminType="";
			       	  if($row['isadmin'] == "1" || $row['isadmin'] == "2" ||  $row['isadmin'] == "3"){ // isAdmin - 1->Financial, isAdmin -2 -> Graduate Admin  
			       	  	 $isAdmin = True;
	             ...
	             if($isAdmin){
					...
					session_destroy();		
				  echo "Staff details not exist in DB, Contact Admin.";			
			    }
		}else{
			 session_destroy();
