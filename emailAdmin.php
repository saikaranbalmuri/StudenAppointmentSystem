...
$stmt2 = $conn->prepare('SELECT R.student_id AS stu_id, Stu.firstname as stu_fn,
 Stu.lastname AS stu_ln,Stu.email AS stu_email,RP.currentpost,R.semester,R.year,RP.salarypaid,RP.startdate,RP.enddate,Sta.firstname As sta_fn,Sta.faculty_id as sta_id,Sta.lastname AS sta_ln,Sta.email AS sta_email,
 R.isreappointed FROM Recruitments1 R join RecruitPositions RP on R.id = RP.recruitment_id JOIN Student Stu ON R.student_id = Stu.uin JOIN Staff Sta ON R.faculty_id = Sta.faculty_id and R.id = ?');
